...
do
    ...
    function BaseGroupboxFuncs:AddESPPreview(Idx, Info)
        local Groupbox = self
        local Container = Groupbox.Container

        local BoxWidth = if typeof(Info.BoxWidth) == "number" then Info.BoxWidth else 70
        local BoxHeight = if typeof(Info.BoxHeight) == "number" then Info.BoxHeight else 120
        local HealthbarWidth = if typeof(Info.HealthbarWidth) == "number" then Info.HealthbarWidth else 5
        local CornerLength = if typeof(Info.CornerLength) == "number" then Info.CornerLength else 0.2
        local HealthbarSide = if Info.HealthbarSide == "Right" then "Right" else "Left"
        local BoxOffset = if typeof(Info.BoxOffset) == "number" then Info.BoxOffset else -10

        local DefaultSkeleton = {
            Points = {},
            Connections = {
                {"Head", "UpperTorso"},
        ...
        local SkeletonConfig = DefaultSkeleton
        if typeof(Info.Skeleton) == "table" then
            SkeletonConfig = {
                Points = if typeof(Info.Skeleton.Points) == "table" then Info.Skeleton.Points else DefaultSkeleton.Points,
                Connections = if typeof(Info.Skeleton.Connections) == "table" then Info.Skeleton.Connections else DefaultSkeleton.Connections,
                Color = Info.Skeleton.Color or DefaultSkeleton.Color,
                Thickness = if typeof(Info.Skeleton.Thickness) == "number" then Info.Skeleton.Thickness else DefaultSkeleton.Thickness,
            }
        end
        ...
        local ChamsConfig = DefaultChams
        if typeof(Info.Chams) == "table" then
            ChamsConfig = {
                FillColor = Info.Chams.FillColor or DefaultChams.FillColor,
                FillTransparency = if typeof(Info.Chams.FillTransparency) == "number" then Info.Chams.FillTransparency else DefaultChams.FillTransparency,
                OutlineTransparency = if typeof(Info.Chams.OutlineTransparency) == "number" then Info.Chams.OutlineTransparency else DefaultChams.OutlineTransparency,
            }
        end

        local DefaultHealthGradient = ColorSequence.new({
        ...
        local DefaultLabels = {
            {
                Name = "Name",
                Text = Info.NameText or (LocalPlayer and LocalPlayer.Name or "Player"),
                Position = "Top",
                Color = Color3.new(1, 1, 1),
                TextSize = 11,
                Font = Enum.Font.Code,
            ...
            {
                Name = "Weapon",
                Text = Info.WeaponText or "Weapon",
                Position = "Bottom",
                Color = Color3.new(1, 1, 1),
                TextSize = 11,
                Font = Enum.Font.Code,
        ...
        local LabelConfigs = if typeof(Info.Labels) == "table" then Info.Labels else DefaultLabels

        local ESPPreview = {
            Height = if typeof(Info.Height) == "number" and Info.Height > 0 then Info.Height else 200,
            Visible = if Info.Visible ~= nil then Info.Visible else true,
            BoxColor = Info.BoxColor or Color3.fromRGB(255, 255, 255),
            HealthPercent = if typeof(Info.HealthPercent) == "number" then Info.HealthPercent else 1,
            HealthGradient = Info.HealthGradient or DefaultHealthGradient,
            Type = "ESPPreview",
        ...
        if not PreviewObject then
            PreviewObject = Instance.new("Model")
            PreviewObject.Name = "ESPPreviewDummy"
            ...
            Head.Parent = PreviewObject

            PreviewObject.PrimaryPart = Root
        end
        ...
        local Holder = Library:Create("Frame", {
            BackgroundTransparency = 1,
            Size = UDim2.new(1, -4, 0, ESPPreview.Height),
        ...
        Library:GiveSignal(InputService.InputChanged:Connect(function(input)
            ...
            if input.UserInputType == Enum.UserInputType.MouseMovement then
                local MouseDelta = input.Position - LastMousePos
                LastMousePos = input.Position
                local Position = PreviewObject:GetPivot().Position
                local RotationY = CFrame.fromAxisAngle(Vector3.new(0, 1, 0), -MouseDelta.X * 0.01)
                PreviewCamera.CFrame = CFrame.new(Position) * RotationY * CFrame.new(-Position) * PreviewCamera.CFrame
                local RotationX = CFrame.fromAxisAngle(PreviewCamera.CFrame.RightVector, -MouseDelta.Y * 0.01)
                local PitchedCFrame = CFrame.new(Position) * RotationX * CFrame.new(-Position) * PreviewCamera.CFrame
                if PitchedCFrame.UpVector.Y > 0.1 then
                    PreviewCamera.CFrame = PitchedCFrame
                end
            end
        end))
        ...
        ViewportFrame.InputChanged:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseWheel then
                local ZoomAmount = input.Position.Z * 2
                PreviewCamera.CFrame += PreviewCamera.CFrame.LookVector * ZoomAmount
            end
        end)
        ...
        local CornerBox = Library:Create("Frame", {
            BackgroundTransparency = 1,
            Size = UDim2.fromOffset(BoxWidth, BoxHeight),
            Position = UDim2.new(0.5, -BoxWidth / 2, 0.5, -BoxHeight / 2 + BoxOffset),
            Visible = false,
            ZIndex = 10,
            Parent = Overlay,
        })

        local function CreateCornerLine(Parent, PosX, PosY, SizeX, SizeY, Anchor)
            local IsHorizontal = SizeX > SizeY
            local Line = Library:Create("Frame", {
                BackgroundColor3 = ESPPreview.BoxColor,
                BorderSizePixel = 0,
                Position = UDim2.new(PosX, 0, PosY, 0),
                Size = if IsHorizontal then UDim2.new(SizeX, 0, 0, 1) else UDim2.new(0, 1, SizeY, 0),
        ...
        local CornerLines = {}
        CornerLines[1] = CreateCornerLine(CornerBox, 0, 0, CornerLength, 0, Vector2.zero)
        CornerLines[2] = CreateCornerLine(CornerBox, 0, 0, 0, CornerLength, Vector2.zero)
        CornerLines[3] = CreateCornerLine(CornerBox, 1, 0, CornerLength, 0, Vector2.new(1, 0))
        CornerLines[4] = CreateCornerLine(CornerBox, 1, 0, 0, CornerLength, Vector2.new(1, 0))
        CornerLines[5] = CreateCornerLine(CornerBox, 0, 1, CornerLength, 0, Vector2.new(0, 1))
        CornerLines[6] = CreateCornerLine(CornerBox, 0, 1, 0, CornerLength, Vector2.new(0, 1))
        CornerLines[7] = CreateCornerLine(CornerBox, 1, 1, CornerLength, 0, Vector2.new(1, 1))
        CornerLines[8] = CreateCornerLine(CornerBox, 1, 1, 0, CornerLength, Vector2.new(1, 1))
        ...
        Library:Create("UIStroke", {
            Color = ESPPreview.BoxColor,
            Thickness = 1,
            LineJoinMode = Enum.LineJoinMode.Miter,
            Parent = FullBox,
        })

        local FullBoxInner = Library:Create("Frame", {
            BackgroundTransparency = 1,
        ...
        if HealthbarSide == "Left" then
            HealthbarPosX = -BoxWidth / 2 - HealthbarWidth - 3
        else
            HealthbarPosX = BoxWidth / 2 + 3
        end

        local HealthbarBg = Library:Create("Frame", {
            BackgroundColor3 = Color3.new(0, 0, 0),
            BorderSizePixel = 0,
            Size = UDim2.fromOffset(HealthbarWidth, BoxHeight),
            Position = UDim2.new(0.5, HealthbarPosX, 0.5, -BoxHeight / 2 + BoxOffset),
            Visible = false,
            ZIndex = 10,
            Parent = Overlay,
        })
        ...
        local HealthbarFill = Library:Create("Frame", {
            ...
            BorderSizePixel = 0,
            Size = UDim2.new(1, -2, ESPPreview.HealthPercent, -2),
            Position = UDim2.new(0, 1, 1 - ESPPreview.HealthPercent, 1),
            ZIndex = 11,
            Parent = HealthbarBg,
        })
        ...
        Library:Create("UIGradient", {
            Rotation = 90,
            Color = ESPPreview.HealthGradient,
            Parent = HealthbarFill,
        })

        local LabelInstances = {}
        local TopLabelCount = 0
        local BottomLabelCount = 0
        ...
        for _, LabelConfig in ipairs(LabelConfigs) do
            ...
            if LabelConfig.Position == "Top" then
                PosY = -BoxHeight / 2 + BoxOffset - 16 - (TopLabelCount * 14) + LabelOffset
                TopLabelCount = TopLabelCount + 1
            else
                PosY = BoxHeight / 2 + BoxOffset + 2 + (BottomLabelCount * 14) + LabelOffset
                BottomLabelCount = BottomLabelCount + 1
            end

            local Label = Library:Create("TextLabel", {
                BackgroundTransparency = 1,
                Size = UDim2.fromOffset(BoxWidth, 14),
                Position = UDim2.new(0.5, -BoxWidth / 2, 0.5, PosY),
                TextColor3 = LabelConfig.Color or Color3.new(1, 1, 1),
                TextSize = LabelConfig.TextSize or 11,
                Font = LabelConfig.Font or Enum.Font.Code,
                Text = LabelConfig.Text or "",
                TextStrokeTransparency = 0,
            ...
            LabelInstances[LabelConfig.Name] = {
                Instance = Label,
                Config = LabelConfig,
                BaseY = PosY,
            }

            ESPPreview.LabelStates[LabelConfig.Name] = false
        end

        local SkeletonFrame = Library:Create("Frame", {
            BackgroundTransparency = 1,
            Size = UDim2.fromScale(1, 1),
            Visible = false,
            ZIndex = 10,
        ...
        PreviewChams.FillColor = ChamsConfig.FillColor
        ...
        local BoxTypes = {}
        if typeof(Info.BoxTypes) == "table" then
            for _, V in ipairs(Info.BoxTypes) do
                BoxTypes[V] = true
            end
        else
            BoxTypes["Corner"] = true
            BoxTypes["Box"] = true
        end

        function ESPPreview:SetBoxes(Enabled, Type)
            ESPPreview.ShowBoxes = Enabled
            ESPPreview.BoxType = Type or ESPPreview.BoxType
            CornerBox.Visible = Enabled and ESPPreview.BoxType == "Corner" and BoxTypes["Corner"] ~= nil
            FullBox.Visible = Enabled and ESPPreview.BoxType == "Box" and BoxTypes["Box"] ~= nil
        end

        function ESPPreview:SetLabel(Name, Enabled)
            ESPPreview.LabelStates[Name] = Enabled
            if LabelInstances[Name] then
                LabelInstances[Name].Instance.Visible = Enabled
            end
        end

        function ESPPreview:SetLabelText(Name, Text)
            if LabelInstances[Name] then
                LabelInstances[Name].Instance.Text = Text
                LabelInstances[Name].Config.Text = Text
            end
        end

        function ESPPreview:SetLabelColor(Name, Color)
            if LabelInstances[Name] then
                LabelInstances[Name].Instance.TextColor3 = Color
                LabelInstances[Name].Config.Color = Color
            end
        end
        ...
        function ESPPreview:SetBoxColor(Color)
            ...
            local Stroke = FullBox:FindFirstChildOfClass("UIStroke")
            if Stroke then
                Stroke.Color = Color
            end
        end
        ...
        function ESPPreview:SetChamsColor(Color)
            ChamsConfig.FillColor = Color
            PreviewChams.FillColor = Color
        end

        function ESPPreview:SetChamsFillTransparency(Value)
            ChamsConfig.FillTransparency = Value
            PreviewChams.FillTransparency = Value
        end

        function ESPPreview:SetSkeletonColor(Color)
            SkeletonConfig.Color = Color
            for _, Line in pairs(BoneLines) do
                Line.BackgroundColor3 = Color
            end
        end

        function ESPPreview:SetHealth(Percent)
            ESPPreview.HealthPercent = math.clamp(Percent, 0, 1)
            HealthbarFill.Size = UDim2.new(1, -2, ESPPreview.HealthPercent, -2)
        ...
        function ESPPreview:SetVisible(Visible)
            ...
            if Blank then Blank.Visible = Visible end
            Groupbox:Resize()
        end

        function ESPPreview:GetLabel(Name)
            return LabelInstances[Name]
        end

        function ESPPreview:GetOverlay()
            return Overlay
        end

        Blank = Groupbox:AddBlank(10, ESPPreview.Visible)
        Groupbox:Resize()

        ESPPreview.Holder = Holder
        ESPPreview.Container = Container
        ESPPreview.Overlay = Overlay
        ESPPreview.PreviewObject = PreviewObject
        ESPPreview.PreviewCamera = PreviewCamera
